...
-   **式子(1)**  种子周期影响 ：$$1 - 10^(-0.25 \times i)$$ 
-   **式子(2)**  做种人数影响：$$1 + 1.414 \times 10^{-0.167 \times (i -1)}$$
...
（3）我们让做种数和周期数值相等，$$式(1) \times 式(2) $$ 的函数图形为:
...
（4）我们让做种数、大小和周期的数值相等，$$式(1) \times 式(2)$$的图形如下：
...
当周期大于8，种子数多于10，$$式(1) \times 式(2)$$的值不再随他们的增加而明显改变，因此以此时的值代替周期大于8，种子数多于10的值是合理的。再分析满足以上情况时的魔力值。首先周期等于8，种子数等于10时，$$式(1) \times 式(2)$$ 的值分别为$$0.99$$和$$1.04$$，则$$A$$值为$$A=1.0296 \times Si$$ ,

$$B$$值为$$B= 31.85\times arctan\frac{1.0296\times S_i}{300}$$其图形如下:
...
同时我们还可以考虑得到一个重要结论：另外，占用同样的做种磁盘空间，多个种子和一个种子所获得的魔力值是相等的(不考虑$$0.6 \times 12$$ ，适用于周期大于8，做种数多于10 的情况)，因为他们的 $$A$$ 值是相等的，比如一个100GB 种子和 1GB 种子 100个，他们A值可用 $$A = 1.0296 \times S_i$$ 计算，显然二者相等。
